...
export default function PanelGraph({ graphData, showGap, setShowGap }) {
  ...
  useEffect(() => {
    ...
    const isGap  = (d) => showGap && graphData.gap_skills?.includes(d.id);
    ...
    <div className="panel-card">
      ...
      <div style={{ flex: 1, overflow: 'hidden', position: 'relative' }}>
        ...
        <div style={{
          ...
          {graphData.nodes.length}N · {graphData.links.length}L · {Math.round(zoom * 100)}%
        </div>
      </div>
    </div>
  );
